...
class CloneContentTypeForm extends FormBase {
  ...
  protected function buildOperationsList(FormStateInterface $form_state) {
    ...
    $operations[] = ['\Drupal\content_type_clone\Form\CloneContentType::cloneContentType', [$values]];
    ...
    foreach ($fields as $field) {
      if (!empty($field->getTargetBundle())) {
        ...
        $operations[] = ['\Drupal\content_type_clone\Form\CloneContentType::cloneContentTypeField', [$data]];
      }
    }
    ...
    if ((int)$values['copy_source_nodes'] == 1) {
      ...
      foreach ($nids as $nid) {
        if ((int) $nid > 0) {
          $operations[] = ['\Drupal\content_type_clone\Form\CloneContentType::copyContentTypeNode', [$nid, $values]];
        }
      }
    }
